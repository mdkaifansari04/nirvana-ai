      ...
      <div className="w-full overscroll-x-hidden min-h-screen bg-slate-50">
         ...
         <div className="relative z-10 container mx-auto px-4 py-12">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-16">
               <h1 className="text-5xl md:text-6xl font-semibold bg-primary bg-clip-text text-transparent mb-6">Welcome to Your Self-Care Lab 🧪</h1>
               <p className="text-xl max-w-2xl tracking-tight mx-auto leading-relaxed text-secondary-foreground">Explore tools to boost your mood, relax your mind, and grow mentally stronger.</p>
            </motion.div>
         ...
         >
            ...
            >
               ...
               <div className={`text-center space-y-2 ${feature.comingSoon ? 'opacity-50' : ''}`}>
                  <h3 className="text-2xl font-bold text-gray-800 group-hover:text-gray-900 transition-colors">{feature.title}</h3>
                  <p className="text-gray-600 leading-5 text-sm">{feature.description}</p>
               </div>

               {/* Hover Arrow */}
               {!isLoading && (
                  <motion.div className="absolute bottom-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300" whileHover={{ x: 4 }}>
