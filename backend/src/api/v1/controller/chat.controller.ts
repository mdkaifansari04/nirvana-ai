...
export const chat = async (req: CustomRequest, res: Response, next: NextFunction) => {
   try {
      ...
      const systemPrompt = CHATBOT_SYSTEM_PROMPTS[chatbotName].system_prompt + CHAT_SYSTEM_PROMPT;
...
export const startChatSession = async (req: CustomRequest, res: Response, next: NextFunction) => {
   try {
      ...
      if (!query) return next(new ErrorResponse('Error occured while creating the session.', 400));
