...
import CurrentStreakLoadingView, {
  WordSkeletonLoadingView,
} from "@/components/shared/loading-view";
...
import {
  chartConfig,
  getActivityData,
  getAverageLengthByMonth,
  getCurrentStreak,
  getEntriesByMonth,
  getEntriesWithWordCount,
} from "@/lib/demo-journal-data";
      ...
      <div className="grid gap-6 mb-8">
              ...
              <Card className="overflow-hidden border-none shadow-md">
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {entriesByMonth[entriesByMonth.length - 1].entries}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Entries in {entriesByMonth[entriesByMonth.length - 1].month}
                  </p>
                </CardContent>
              </Card>
            </div>
        ...
        <Card className="border-none shadow-md">
          <CardHeader>
            ...
            <CardDescription>
              Your journaling frequency over the past 6 months
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <div className="sm:min-w-0 px-4 sm:px-6 md:px-8">
                  <div className="activity-calendar-grid grid gap-1">
                    {activityData.map((day) => (
                      <div
                        key={day.date}
                        className={`activity-calendar-cell h-3 w-3 rounded-sm border ${
                          day.level === 0
      ...
      <div className="grid gap-6">
        <Card className="border-none shadow-md">
          <CardHeader>
            ...
            <CardDescription>
              Your personal reflection and writing activities
            </CardDescription>
          </CardHeader>
          <CardContent>
            <QueryWrapper
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {sortedEntries.slice(0, visibleEntries).map((entry) => (
                      <Link
                        key={entry._id}
                        href={`/dashboard/journal/${entry._id}`}
                        className="block"
                      >
