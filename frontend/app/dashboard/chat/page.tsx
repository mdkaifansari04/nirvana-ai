...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] bg-background">
      <MessagesSidebar resetChat={resetChat} selectedTopicId={selectedTopicId} onTopicSelect={setSelectedTopicId} />
      {!selectedTopicId ? (
        <BlankChat />
      ) : (
        <>
          <div className="flex-1 flex flex-col">
                ...
                {Array.isArray(chat) &&
                  chat?.map((chat, i) => (
                    <ChatBubble key={`${chat.message}-${i}`} variant={chat.sender === "user" ? "sent" : "received"} className={chat.sender === "user" ? "max-w-[85%]" : ""}>
                      <ChatBubbleAvatar src={chat.sender === "user" ? "U" : "A"} fallback={chat.sender === "user" ? "U" : "A"} className={`h-8 w-8 relative -top-7 ${chat.sender === "model" ? "bg-secondary" : ""}`} />
                      <ChatBubbleMessage variant={chat.sender === "user" ? "sent" : "received"} className={chat.sender === "user" ? "bg-primary text-primary-foreground rounded-[20px] rounded-tr-none" : "bg-secondary text-secondary-foreground rounded-[20px] rounded-tl-none"}>
                        {chat.message}
                      </ChatBubbleMessage>
                    </ChatBubble>
                  ))}
            ...
            <div className="p-4 border-t bg-background">
              <div className="relative border rounded-lg">
                <ChatInput onChange={(e) => setPrompt(e.target.value)} value={prompt} placeholder="Send your message to Dr. freud..." className="bg-card border-0 shadow-none pr-12 rounded-2xl" />
                <button onClick={handleSubmit} type="button" className="cursor-pointer absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-full bg-primary text-primary-foreground hover:bg-primary/90 transition-colors" aria-label="Send message">
  ...
  return (
    <div className="flex-1 flex flex-col h-full justify-center items-center">
      <div className="flex justify-center items-center">
        <h5>Please select a topic to start chatting</h5>
      </div>
    </div>
  );
}
