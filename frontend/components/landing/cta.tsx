    ...
    <section className="relative bg-white border border-amber-900 text-black px-6 py-20 rounded-[2.5rem] mt-16 sm:mx-8 md:mx-16 shadow-lg overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto text-center space-y-6">
        ...
        <p className="text-lg text-black/90 max-w-2xl mx-auto">
          Step into a world of empathic AI companions, personalized support, and
          evidence-based mental health tools — available anytime, anywhere.
        </p>
        <div className="flex flex-row items-center justify-center gap-4 mt-6">
          <Link
            href="/dashboard"
            className="font-semibold px-8 py-3 rounded-full hover:bg-white/50 transition-colors shadow"
          >
            Dashboard
          </Link>
          <Link
            href="/sign-up"
            className="bg-primary text-white font-semibold px-8 py-3 rounded-full hover:bg-primary/90 transition-colors shadow"
          >
            Get Started
          </Link>
        </div>
        <p className="text-md text-black/80">
          Nirvana.Ai — Empathy. Science. You.
        </p>
      </div>
    </section>
  );
