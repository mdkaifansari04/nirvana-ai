...
const ExerciseContentSchema = new mongoose.Schema({
	qna: {
		type: [QnASchema],
		validate: [
			(val: (typeof QnASchema)[]) => val.length === 2,
			"Exactly 2 QnA items required",
		],
		required: true,
	},
	mcq: {
		type: [MCQSchema],
		validate: [
			(val: (typeof MCQSchema)[]) => val.length === 5,
			"Exactly 5 MCQ items required",
		],
		required: true,
	},
});
...
const MicroExerciseSchema = new mongoose.Schema(
	{
		...
		ai_generated_report: { type: ReportSchema, required: true },
	},
	{
		timestamps: true,
	},
...
export const MicroExercise = mongoose.model(
	"micro-exercise",
	MicroExerciseSchema,
);
