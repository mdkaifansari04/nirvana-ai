         ...
         <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden lg:flex flex-1 justify-center gap-10">
               {navItems.map(({ name, href }) => (
                  <a key={name} href={href} className="text-white font-semibold tracking-wider hover:text-amber-200 transition">
                     {name}
                  </a>
               ))}
            </nav>
            ...
            <div className="hidden lg:flex items-center gap-4">
               {user ? (
                  <>
                     ...
                     <button onClick={handleLogout} className="bg-amber-700 hover:bg-amber-600 text-white font-semibold text-sm px-4 py-2 rounded-lg transition shadow-sm">
                        Logout
                     </button>
                  </>
               ) : (
            ...
            <button className="lg:hidden size-10 flex flex-col justify-center items-center gap-[5px] rounded-lg border border-white/20 bg-white/10 backdrop-blur-sm p-2">
