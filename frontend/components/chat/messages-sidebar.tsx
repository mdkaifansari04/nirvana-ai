      ...
      <QueryWrapper
        ...
        view={
          <>
            {chatbots &&
              chatbots.map((topic) => (
                <button key={topic._id} onClick={() => handleTopicSelect(topic._id)} type="button" aria-pressed={topic._id === selectedTopicId} className={`border-b flex w-full items-center gap-3 px-6 py-3 cursor-pointer hover:bg-accent/50 transition-colors ${topic._id === selectedTopicId ? "bg-accent/50" : ""} text-left`}>
