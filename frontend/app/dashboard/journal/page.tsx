                ...
                <div className="sm:min-w-0 px-4 sm:px-6 md:px-8">
                  <div className="activity-calendar-grid grid gap-1">
                    {activityData.map((day) => (
                      <div key={day.date} className={`activity-calendar-cell h-3 w-3 rounded-sm border ${day.level === 0 ? "bg-muted hover:bg-muted-foreground/30" : "bg-foreground dark:bg-foreground"}`} title={`${day.date}: ${day.count} entries`} />
                    ))}
                  </div>
                </div>
              </div>
