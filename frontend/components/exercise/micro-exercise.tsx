...
interface MicroExerciseProps {
   ...
   onComplete: (data: {
      qnaAnswers: { [index: number]: string };
      mcqAnswers: { [index: number]: string[] };
...
export function MicroExercise({ exerciseContent, sessionGoal, initialMoodRating, initialEmotion, onComplete }: MicroExerciseProps) {
   ...
   const totalSteps = 1 + exerciseContent.mcq.length + 1;
   ...
   const isStepValid = () => {
      if (currentStep === 0) {
         // QnA step
         return Object.keys(qnaAnswers).length === exerciseContent.qna.length && Object.values(qnaAnswers).every((answer) => answer.trim() !== '');
      }
         ...
         <CardContent className="space-y-6">
            {currentStep === 0 && (
               <div className="space-y-6">
                  ...
                  {exerciseContent.qna.map((qa, index) => (
            ...
            {currentStep > 0 && currentStep < totalSteps - 1 && (
               <div>
                  <h3 className="text-lg font-medium mb-3">
                     Question {currentStep} of {exerciseContent.mcq.length}
                  </h3>
                  {currentStep - 1 < exerciseContent.mcq.length ? (
                     <MultipleChoice
                        question={exerciseContent.mcq[currentStep - 1].question}
                        options={exerciseContent.mcq[currentStep - 1].options}
                        selectedOptions={mcqAnswers[currentStep - 1] || []}
                        onChange={(options) => updateMCQAnswer(currentStep - 1, options)}
                     />
                  ) : (
